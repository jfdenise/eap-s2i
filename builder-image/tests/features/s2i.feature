Feature: EAP s2i tests
  ...
  Scenario: Build the image with a server
    Given s2i build https://github.com/jfdenise/eap-s2i from test/test-app with env and True using main
    | variable                             | value         |
  ...
  Scenario: Test extension called at startup.
    Given s2i build https://github.com/jfdenise/eap-s2i from test/test-app-extension with env and true using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
    Then container log should contain WFLYSRV0025
  ...
  Scenario: Test extension called at build time, copy a file inside JBOSS_HOME.
    Given s2i build https://github.com/jfdenise/eap-s2i from test/test-app-extension2 with env and true using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
    Then container log should contain WFLYSRV0025
  ...
  Scenario: Test custom settings with galleon
    Given s2i build https://github.com/jfdenise/eap-s2i from test/test-app-settings with env and true using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
  ...
  Scenario: Test execution of user CLI operations at S2I phase
    Given s2i build https://github.com/jfdenise/eap-s2i from test/vanilla-eap/test-app-s2i-cli-scripts with env and true using main
     | variable                               | value                                                 |
  ...
  Scenario: Test jaxrs-server -jpa +jpa-distributed
    Given s2i build https://github.com/jfdenise/eap-s2i from test/test-app-jpa2lc with env and True using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
    Then container log should contain WFLYSRV0025
  ...
  Scenario: Test jaxrs-server +ejb-lite, -ejb-local-cache +ejb-dist-cache. Verify JGroups configuration added by ejb-dist-cache
    Given s2i build https://github.com/jfdenise/eap-s2i from test/test-app-ejb with env and True using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
    Then container log should contain WFLYSRV0025
    Then check that page is served
  ...
  Scenario: Test building and running slim application
    Given s2i build https://github.com/jfdenise/eap-s2i  from test/test-app-slim with env and true using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
    Then container log should contain WFLYSRV0025

  Scenario: Test failing packaging.
    Given failing s2i build https://github.com/jfdenise/eap-s2i from test/test-app-invalid using main
    | variable                             | value         |
    ### PLACEHOLDER FOR CLOUD CUSTOM TESTING ###
